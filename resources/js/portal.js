(() => {
    ...
    if (autosave) {
        let timer; let saving = false; let queued = false; let dirty = false; let revision = 0; let conflicted = false; let submitting = false; const status = autosave.querySelector('[data-autosave-status]'); const timestamp = autosave.querySelector('[name="updated_at"]');
        const save = async () => {
            ...
            const data = Object.fromEntries([...new FormData(autosave).entries()].filter(([, value]) => !(value instanceof File))); ['keywords','references'].forEach((key) => { if (typeof data[key] === 'string') data[key] = data[key].split(key === 'keywords' ? ',' : '\n').map((v) => v.trim()).filter(Boolean); });
            try { const response = await fetch(autosave.dataset.autosave, { method: 'PATCH', headers: {'Accept':'application/json','Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]')?.content}, body: JSON.stringify(data), signal: AbortSignal.timeout(20000) }); const result = await response.json();
                ...
                if (!response.ok) { const error = new Error(result.message || 'Autosave failed. Keep this page open and try saving again.'); error.code = result.code || `http_${response.status}`; throw error; } if (timestamp) timestamp.value = result.updated_at; dirty = revision !== savingRevision; if (status) { status.textContent = dirty ? 'Unsaved changes' : `Saved ${new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}`; status.style.color = ''; }
        ...
        autosave.addEventListener('input', () => { dirty = true; revision++; clearTimeout(timer); if (conflicted) return; if (status) status.textContent = 'Unsaved changes'; timer = setTimeout(() => { timer = null; save(); }, 1800); });
        ...
        window.addEventListener('beforeunload', (event) => { if (!submitting && (dirty || saving || timer || [...autosave.querySelectorAll('input[type=file]')].some((input) => input.files.length))) { event.preventDefault(); event.returnValue = ''; } });
    }
