...
ready(() => {
    ...
    const scrollNavBrand = document.querySelector('[data-scroll-nav-brand]');
    ...
    const updateHeader = () => {
        ...
        if (scrollNavBrand) {
            scrollNavBrand.hidden = !isScrolled;
            scrollNavBrand.style.display = isScrolled ? '' : 'none';
        }
    };
    updateHeader();
    ...
    document.querySelectorAll('a[href^="#"]').forEach((link) => {
        link.addEventListener('click', (event) => {
            ...
            window.scrollTo({ top: Math.max(0, targetPosition), behavior: reducedMotion.matches ? 'auto' : 'smooth' });
    ...
    if (liveStats?.dataset.statsUrl) {
        const refreshStats = async () => {
            try {
                const response = await fetch(liveStats.dataset.statsUrl, { headers: { Accept: 'application/json' }, cache: 'no-store' });
                if (!response.ok) return;
                const stats = await response.json();
                Object.entries(stats).forEach(([key, value]) => {
                    const target = liveStats.querySelector(`[data-live-stat="${key}"]`);
                    if (target && Number.isFinite(Number(value))) target.textContent = Number(value).toLocaleString();
                });
            } catch {
                // Leave the server-rendered figures in place if the request is unavailable.
            }
        };
        refreshStats();
