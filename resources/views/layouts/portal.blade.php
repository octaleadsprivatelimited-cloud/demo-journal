...
<head>
    ...
    <meta name="robots" content="noindex,nofollow"><title>@yield('title', 'Workspace') — {{ config('app.name', 'Singapore Journal of Cardiology') }}</title>
    <link rel="icon" href="{{ asset('assets/sjc-logo-transparent.png') }}?v=sjc-20260903" type="image/png">
    <link rel="apple-touch-icon" href="{{ asset('assets/sjc-logo-transparent.png') }}?v=sjc-20260903">
...
<div class="portal-shell" data-portal-shell>
    <aside class="portal-sidebar" id="portal-sidebar" data-portal-sidebar>
        <div class="portal-brand"><a href="{{ route('home') }}"><img class="portal-logo" src="{{ asset('assets/sjc-logo-transparent.png') }}" alt="SJC International Journal"><span><strong>{{ config('app.name', 'Singapore Journal of Cardiology') }}</strong><small>Editorial workspace</small></span></a><button type="button" data-sidebar-close aria-label="Close navigation"><x-portal.icon name="close" /></button></div>
...
@else
<main class="auth-shell"><a class="auth-brand" href="{{ route('home') }}"><img class="portal-logo" src="{{ asset('assets/sjc-logo-transparent.png') }}" alt="SJC International Journal"><span><strong>{{ config('app.name', 'Singapore Journal of Cardiology') }}</strong><small>Journal of ideas &amp; inquiry</small></span></a><section class="auth-card"><div class="auth-card-head"><p class="portal-eyebrow">@yield('auth-eyebrow', 'Secure journal access')</p><h1>@yield('page-title', 'Welcome')</h1><p>@yield('page-description')</p></div><x-portal.flash />@yield('content')</section><p class="auth-foot"><a href="{{ route('home') }}">← Return to the journal</a></p></main>
@endauth
