...
@section('page-title', 'Join our contributor community')
@section('page-description', 'Create a verified profile and begin preparing your first manuscript.')
@section('content')
<style>.auth-card{width:min(100%,760px)}</style>
<form class="portal-form" method="post" action="{{ route('register.store') }}" enctype="multipart/form-data" data-loading>@csrf
    <div class="form-grid">
        ...
        <div class="portal-field"><label for="designation">Designation</label><input class="portal-input" id="designation" name="designation" value="{{ old('designation') }}" placeholder="Research Fellow, Editor, Professor…"><x-portal.field-error name="designation" /></div>
    ...
    <button class="portal-button primary" type="submit" data-loading-text="Creating account…">Create author account</button>
    <div class="auth-links"><span>Already registered? <a href="{{ route('login') }}">Sign in</a></span></div>
</form>
@endsection
