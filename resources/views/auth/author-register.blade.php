...
<form class="portal-form" method="post" action="{{ route('author.register.store') }}" enctype="multipart/form-data" data-loading>@csrf
    <div class="form-grid">
        ...
        <div class="portal-field"><label for="profile_image">Profile image <span>JPG, PNG, WebP or SVG · lossless compression, 1 MB maximum</span></label><input class="portal-input" id="profile_image" name="profile_image" type="file" accept="image/jpeg,image/png,image/webp,image/svg+xml"><x-portal.field-error name="profile_image" /></div>
