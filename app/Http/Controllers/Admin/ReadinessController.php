...
class ReadinessController extends Controller {
 ...
 private function validated(Request $r,string $x):array {return match($x){
  ...
  'issues'=>$r->validate(['journal_volume_id'=>'required|exists:journal_volumes,id','number'=>'required|string|max:40','title'=>'nullable|string|max:255','description'=>'nullable|string|max:4000','cover'=>'nullable|image|mimes:jpg,jpeg,png,webp|max:4096','publication_date'=>'nullable|date','is_current'=>'sometimes|boolean']),
  'editorial-members'=>$r->validate(['name'=>'required|string|max:180','group'=>'required|in:editorial_board,reviewers,advisors,publisher_staff','role'=>'required|string|max:120','institution'=>'nullable|string|max:255','department'=>'nullable|string|max:255','country'=>'nullable|string|max:120','credentials'=>'nullable|string|max:255','biography'=>'nullable|string|max:10000','orcid'=>['nullable','regex:/^\d{4}-\d{4}-\d{4}-\d{3}[\dX]$/'],'photo'=>'nullable|image|mimes:jpg,jpeg,png,webp|max:4096','sort_order'=>'nullable|integer|min:0','is_active'=>'sometimes|boolean']),
  'indexing-services'=>$r->validate(['name'=>'required|string|max:180','description'=>'nullable|string|max:2000','official_url'=>'nullable|url:http,https|max:2048','logo'=>'nullable|image|mimes:jpg,jpeg,png,webp|max:4096','status'=>'required|in:pending_verification,verified,not_indexed','sort_order'=>'nullable|integer|min:0','is_active'=>'sometimes|boolean']),
