...
class ImageUploadOptimizer
{
    ...
    public function optimize(UploadedFile $file, string $field): void
    {
        $fail = fn (string $message) => throw ValidationException::withMessages([$field => $message]);
        if (! $file->isValid()) {
            $fail('The file upload failed or exceeds the server upload limit.');
        }
        ...
        if (filesize($file->getPathname()) > 25 * 1024 * 1024) {
            $fail('Images must be valid uploads smaller than 25 MB before compression.');
        }
        if ($svg) {
            if (filesize($file->getPathname()) > self::MAX_BYTES) {
                $fail('SVG files must be no larger than 1 MB. Vector data is preserved without rasterization.');
            }
            $this->validateSvg($file->getPathname(), $fail);
        } elseif (! in_array($mime, ['image/jpeg', 'image/png', 'image/webp'], true)) {
            $fail('Supported images are JPG, PNG, WebP and self-contained SVG.');
        } else {
            $size = @getimagesize($file->getPathname());
            if (! $size || $size[0] * $size[1] > 40000000) {
                $fail('The image is invalid or exceeds the 40-megapixel processing limit.');
            }
            $command = match ($mime) {
            ...
            if ($command) {
                ...
                try {
                    ...
                    $fail('Image compression could not finish. Please try a smaller image.');
                }
            }
        }
        clearstatcache(true, $file->getPathname());
        if (filesize($file->getPathname()) > self::MAX_BYTES) {
            $fail('This image cannot fit within 1 MB using lossless compression. Upload a smaller original; pixels, resolution and quality are never reduced automatically.');
        }
    }
    ...
    private function validateSvg(string $path, callable $fail): void
    {
        ...
        try {
            ...
            if ($xml === '' || stripos($xml, '<!DOCTYPE') !== false || stripos($xml, '<!ENTITY') !== false || ! $document->loadXML($xml, LIBXML_NONET) || $document->doctype) {
                $fail('Upload a valid SVG without document types or external entities.');
            }
            if ($document->documentElement?->localName !== 'svg' || $document->documentElement?->namespaceURI !== 'http://www.w3.org/2000/svg') {
                $fail('The SVG document must have a valid SVG root.');
            }
            ...
            foreach ($document->getElementsByTagName('*') as $element) {
                if ($element->namespaceURI !== 'http://www.w3.org/2000/svg' || ! in_array($element->localName, $allowed, true)) {
                    $fail('SVG must be self-contained: scripts, CSS, animation, embedded images and HTML are not supported. Export a plain SVG.');
                }
                foreach ($element->attributes as $attribute) {
                    ...
                    if (str_starts_with($name, 'on') || in_array($name, ['style', 'base'], true) || ($name === 'href' && ! preg_match('/^#[A-Za-z_][A-Za-z0-9_.:-]*$/D', $value)) || preg_match('/[\\\\@]|(?:javascript|data|https?|file):/i', $value) || (stripos($value, 'url') !== false && ! preg_match('/^url\(#[A-Za-z_][A-Za-z0-9_.:-]*\)$/D', $value))) {
                        $fail('SVG contains active content or external references. Export a self-contained plain SVG.');
                    }
                }
            }
            foreach ((new \DOMXPath($document))->query('//processing-instruction()') as $instruction) {
                $fail('SVG processing instructions are not supported. Export a plain SVG.');
            }
