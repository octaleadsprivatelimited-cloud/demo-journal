...
class ArticleWorkflowService
{
    ...
    public function decide(
        ...
        ?CarbonInterface $revisionDeadline = null,
    ): EditorDecision {
        if (! in_array($result, [
            ArticleStatus::Accepted, ArticleStatus::Approved, ArticleStatus::Rejected,
            ArticleStatus::MinorRevisionRequested, ArticleStatus::MajorRevisionRequested,
            ArticleStatus::RevisionRequired,
        ], true)) {
            throw new DomainException('This is not a valid editorial decision.');
        }

        return DB::transaction(function () use ($article, $actor, $result, $letter, $internalNotes, $revisionDeadline): EditorDecision {
            $article = Article::query()->lockForUpdate()->findOrFail($article->getKey());
            $from = $article->status;
            $this->assertTransition($from, $result);

            $decision = $article->decisions()->create([
                'submission_id' => $article->submissions()->latest('round')->value('id'),
                'decided_by_id' => $actor->getKey(),
                'decision_type' => $result->value,
                'decision_letter' => $letter,
                'internal_notes' => $internalNotes,
                'previous_status' => $from->value,
                'resulting_status' => $result->value,
                'revision_round' => max(1, (int) $article->revision_round),
                'revision_deadline' => $revisionDeadline,
                'decided_at' => now(),
            ]);

            $this->transition($article, $result, $actor, $internalNotes);

            return $decision;
        });
    }

    public function assertReadyForPublication(Article $article): void
    {
        if ($article->status !== ArticleStatus::ReadyForPublication) {
            throw new DomainException('The manuscript is not ready for publication.');
        }

        if (! $article->article_number || ! $article->metadata_validated_at || ! $article->pdf_path) {
            throw new DomainException('Final proof, validated metadata, article number, and publication file are required before publication.');
        }

        $doiRequired = (bool) config('publication.doi_required', false);
        if ($doiRequired && $article->doiRecord?->status !== 'registered') {
            throw new DomainException('A registered DOI is required before publication.');
        }
    }
